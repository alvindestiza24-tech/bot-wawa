...
const FONT_MONO_PATH = join(__dirname, '..', '..', 'storage', 'fonts', 'JetBrainsMono.ttf')
...
async function ensureFont() {
  ...
  const fontDir = dirname(FONT_MONO_PATH)
  if (!existsSync(fontDir)) mkdirSync(fontDir, { recursive: true })
  if (!existsSync(FONT_MONO_PATH)) {
    const { default: axios } = await import('axios')
    const res = await axios.get(FONT_MONO_URL, { responseType: 'arraybuffer', timeout: 20000 })
    writeFileSync(FONT_MONO_PATH, Buffer.from(res.data))
  }
  try {
    const fontBuffer = readFileSync(FONT_MONO_PATH)
    GlobalFonts.register(fontBuffer, FONT_MONO_NAME)
    _fontLoaded = true
  } catch (err) {
    try {
      GlobalFonts.registerFromPath(FONT_MONO_PATH, FONT_MONO_NAME)
      _fontLoaded = true
    } catch (err2) {
      console.error('Gagal mendaftarkan font:', err2.message)
      throw err2
    }
  }
}
...
const JS_KEYWORDS = new Set([
  ...
  'in', 'of', 'class', 'extends', 'super', 'import', 'export', 'default', 'async',
  'await', 'try', 'catch', 'finally', 'throw', 'this', 'void', 'yield', 'from',
  'static', 'get', 'set', 'null', 'undefined',
])

const JS_BUILTINS = new Set([
  'console', 'Math', 'Object', 'Array', 'String', 'Number', 'Boolean', 'Date',
  'JSON', 'Promise', 'Map', 'Set', 'WeakMap', 'WeakSet', 'Symbol', 'Error',
...
const JS_TYPES = new Set([
  'int', 'float', 'string', 'boolean', 'void', 'any', 'never', 'unknown',
...
export async function generateCodeSnap(code, opts = {}) {
  await ensureFont()
  ...
  tempCtx.font = `${FONT_SIZE}px "${FONT_MONO_NAME}"`
  ...
  if (opts.title) {
    ctx.font = `${FONT_SIZE * 0.7}px "${FONT_MONO_NAME}"`
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    if (GUTTER_W > 0) {
      ctx.font = `${Math.round(FONT_SIZE * 0.78)}px "${FONT_MONO_NAME}"`
    ...
    ctx.font = `${FONT_SIZE}px "${FONT_MONO_NAME}"`
