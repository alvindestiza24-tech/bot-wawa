...
export async function generateIQC(username, chatText, avatarSrc, outputPath) {
  await ensureAssets();
  ...
  // Hapus deklarasi OUT yang lama, hanya gunakan yang baru di bawah
  ...
  const OUT = outputPath ?? join(TMP_DIR, `tiktok-chat-${Date.now()}.png`);
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const chatText   = process.argv[3];
  const avatarSrc  = process.argv[4];
  const outputPath = process.argv[5];
  generateIQC(username, chatText, avatarSrc, outputPath);
}