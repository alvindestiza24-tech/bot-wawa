...
export const config_ = {
  name: 'llama8b',
  alias: ['llama8', 'llama31', 'l8'],
  category: 'ai',
  description: 'AI Chat dengan LLaMA 3.1 8B (Groq) - cepat dan ringan',
  usage: '.llama8b <pertanyaan> | .llama8b clear',
  example: '.llama8b apa itu AI?',
  isOwner: false,
  cooldown: 5,
  isEnabled: true,
}
...
export async function handler(m, { sock }) {
  ...
  if (!input) return m.reply('❌ Masukkan pertanyaan. Contoh: *.llama8b apa itu black hole?*')
  ...
  try {
    const raw = await chatCompletion(userMemory, { model: 'llama-3.1-8b-instant', temperature: 0.7, maxTokens: 1024 })
    userMemory.push({ role: 'assistant', content: raw })
    memory.set(m.sender, userMemory)

    const parsed = parseAIResponse(raw)
    const builder = new AIRich(sock).setTitle('⚡ LLaMA 3.1 8B')
