// src/canvas/Iqcbyrin.js (perubahan di bagian akhir)
...
export async function render(text, time, outputPath) {
    await ensureAssets();
    ...
    // ✅ simpan ke storage/.tmp secara default
    const TMP_DIR = join(process.cwd(), 'storage', '.tmp');
    await mkdir(TMP_DIR, { recursive: true });
    const out = outputPath ?? join(TMP_DIR, `iPhone_Chat_Quote_${Date.now()}.png`);
