// src/canvas/welcomecard.js
import { createCanvas, Image } from '@napi-rs/canvas';

const DEFAULT_BG_URL = 'https://files.catbox.moe/p8y6nb.jpg';
const DEFAULT_PFP_URL = 'https://i.imgur.com/bGqSIIq.jpg'; // fallback avatar
...
function drawAvatar(ctx, pfpImage, cfg) {
    ...
    ctx.fillStyle = CONFIG.colors.navy;
    ctx.fill();
...
function drawUnderline(ctx, cfg) {
    ...
    ctx.save();
    ctx.strokeStyle = CONFIG.colors.navy;
...
function drawButton(ctx, text, cfg) {
    ...
    const displayText = text || ' Feel Free To Chat! ';
    ctx.save();
...
function drawFourPointSparkle(ctx, cx, cy, length, rotation = 0) {
    ...
    ctx.quadraticCurveTo(-innerLength, -innerLength, 0, -length);
    ctx.closePath();
    ctx.fillStyle = CONFIG.colors.navyLight;
    ctx.fill();
    ctx.restore();
}
...
function drawAccentLines(ctx, lines) {
    ctx.save();
    ctx.strokeStyle = CONFIG.colors.navyLight;
    ctx.lineCap = 'round';
    lines.forEach(line => {
...
function render(ctx, options, config, bgImage, pfpImage) {
    ...
    ctx.fillText('Welcome', layout.title.x, layout.title.y);
    ctx.restore();
    ...
    drawWrappedText(
        ctx,
        options.description || 'Terima kasih telah bergabung! Jangan ragu untuk bertanya.',
...
export async function generateWelcomeCard(options = {}) {
    // Pastikan background dan avatar selalu ada
