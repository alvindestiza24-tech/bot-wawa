...
const ASSETS_DIR = join(__dirname, 'assets', 'vermeil');
const BG_LOCAL = join(ASSETS_DIR, 'Vermile.jpg');
const FONT_LOCAL = join(ASSETS_DIR, 'Poppins.ttf');

const BG_URL = 'https://raw.githubusercontent.com/Ditzzx-vibecoder/Assets/main/Brat/Vermile.jpg';
...
export async function generateBratVermeil(text) {
  ...
  const tmpDir = join(tmpdir(), `vermeil-${crypto.randomBytes(6).toString('hex')}`);
  ...
  try {
    ...
    // Buat frame secara paralel dalam batch
    const batchSize = 5;
